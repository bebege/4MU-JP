...
{	MyI2CPtr = CI2C1_Init(NULL);
	...
	unsigned short mask = 0x0001; // 初始遮罩是個最高為元為 1 的byte
	char i=0;
	
	  for ( i=0; i<16; ++i)    
    {		Alarm[BMUID][i]=0;  //init
    
        if ((Safety& mask) != 0)   // 如果 b 和 mask 做 AND 的結果不為0
        {   Alarm[BMUID][i]=1;  }    // 即代表 b 的該 bit 為 1
        else                   // 所以顯示一個 1
        {   Alarm[BMUID][i]=0; }    // 否則顯示一個 0
                               // 每次迴圈中都把 mask 右移一個 bit
        mask <<= 1;            // 就成了下個 bit 的遮罩
    }
